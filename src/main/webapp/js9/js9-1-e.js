...
JS9.VERSION = "1-e";		// The version of this namespace
...
JS9.INFOWIDTH = JS9.WIDTH;	// width of js9Info canvas
JS9.INFOHEIGHT = 150;		// height of js9Info canvas
...
JS9.MENUZINDEX = 12;		// z-index of menus: always on top!
...
JS9.TIMEOUT = 500;		// ms before asuming light window is up
// modified from:
...
// global options
JS9.globalOpts = {
    helperType: "nodejs",	// one of: nodejs, get, post
...
// update WCS values for the specified position
JS9.Image.prototype.updateWCS = function(ipos, vstr){
    var im = this;
    var s, t;
    if( im.wcs > 0 ){
	s = JS9.pix2wcs(im.wcs, ipos.x, ipos.y).trim().split(/\s+/);
	t = sprintf("%s(%s, %s)", s[2], s[0], s[1]);
	im.displayMessage("message", vstr + " " + t);
	return this;
    }
    // allow chaining
    return this;
...
JS9.Image.prototype.loadAuxFile = function(type, func){
	...
	if( func ){
	    ...
	    catch(e){ JS9.error("in aux onload callback", e); }
	}
	// debugging
	if( JS9.DEBUG ){
    ...
    errfunc = function(evt){
	JS9.log("could not load auxiliary image: "+aim.id);
    };
	...
	$(aim.png.image).on("load", loadfunc).on("error", errfunc);
...
// display pixel and wcs values (connected to info plugin)
JS9.Image.prototype.displayValues = function(ipos){
    var vstr, val, c;
    var tr = function(s){
	return s.toFixed(3);
    };
	...
	if( this.params.wcssys === "image" ){
	    c = {x: ipos.x, y: ipos.y, sys: "image"};
	} else {
	    c = this.imageToLogicalPos(ipos);
	}
	// get image value: here we need 0-indexed positions
	val = this.raw.data[Math.floor(ipos.y-1) * this.raw.width + 
			    Math.floor(ipos.x-1)];
	// nb: FITS conventions start at pixel 1,1, not 0,0
	switch(this.raw.bitpix){
	case 8:
	    vstr = sprintf("value(%03d) %s(%s, %s)", 
			   val, c.sys, tr(c.x), tr(c.y));
	    break;
	case 16:
	case -16:
	case 32:
	    ...
	    break;
	case -32:
	    vstr = sprintf("value(%.3f) %s(%s, %s)", 
			   val, c.sys, tr(c.x), tr(c.y));
	    break;
	case -64:
	    vstr = sprintf("value(%.6f) %s(%s, %s)", 
			   val, c.sys, tr(c.x), tr(c.y));
	    break;
	default:
	    vstr = sprintf("value(%03d) %s(%s, %s)", 
	...
	if( (this.wcs > 0) && 
	    (this.params.wcssys !== "image") &&
	    (this.params.wcssys !== "physical") ){
	    this.updateWCS(ipos, vstr);
	} else {
	    this.displayMessage("message", vstr);
	}
    }
};
...
JS9.Console = function(){
	...
	this.consoleConjq
	    .css("width", JS9.CONWIDTH)
	    .css("height", JS9.CONHEIGHT);
    }
    // add ability to handle events to this div
...
JS9.Info.init = function(){
    var e;
    // set width and height on div
    this.divjq.css("width", JS9.INFOWIDTH).css("height", JS9.INFOHEIGHT);
    // create DOM canvas element
    this.canvas = document.createElement("canvas");
    // jquery version for event handling and DOM manipulation
    this.canvasjq = $(this.canvas);
    // set class
    this.canvasjq.addClass("JS9Info");
    // how do we allow users to set the size of the canvas??
    // it doesn't go into the CSS and we have no canvas on the Web page ...
    this.canvasjq.attr("height", JS9.INFOHEIGHT);
    this.canvasjq.attr("width", JS9.INFOWIDTH);
    // drawing context
    this.context = this.canvas.getContext("2d");
    // add container to the high-level div
    this.infoConjq = $("<div>")
    ...
    // init message area for this info box
    this.display.initMessages.call(this);
};
...
JS9.Info.display = function(type, message, target){
    var tobj, split, area, tokens, rexp, s, color, info;
    // if image is context
    if( this.display && this.display.pluginInstances ){
	info = this.display.pluginInstances.JS9Info;
    }
    // either display on a separate infobox or on the image
    if( target ){
	tobj = target;
    } else {
	...
	if( this.primary &&
		...
		break;
	    case "message":
		color = JS9.textColorOpts.message;
		break;
	    default:
		color = JS9.textColorOpts.message;
		break;
    ...
    s = message;
    if( split !== "" ){
	tokens = message.split(split);
	if( tokens.length > 2 ){
	    rexp = new RegExp(split, "g");
	    s = message.replace(rexp, "<br>");
	}
    }
    // display the message
...
JS9.Info.clearMain = function(im){
    if( im ){
	im.displayMessage("message", "", im.display);
	...
	$.contextMenu({
		...
		return {
			...
			if( tim ){
				...
				// don't know how to load the dataPath value
				// into the form via an onload function ...
...
JS9.Helper = function(){
    ...
    this.type = JS9.globalOpts.helperType || "nodejs";
    // make the connection
    this.connect();
};
...
JS9.Helper.prototype.connectinfo = function(){
    ...
    if( JS9.helper.connected ){
	return sprintf("connected %s %s", JS9.helper.type, JS9.helper.url);
    }
...
JS9.Fabric.newShapeLayer = function(layerName, layerOpts, divjq){
	...
	if( opts.target && dlayer.display.image ){
		...
		if( !obj.params.winid ){
		    ...
		    //  how to load values into the form via onload?
		    window.setTimeout(function(){
			if( obj.pub ){
    ...
    // only do this if we are in a dhtml light window
    if( dlayer.divjq.closest(".drag-contentarea").length ){
...
JS9.Fabric.addShapes = function(layerName, shape, opts){
	...
	case "polygon":
	    ...
	    s = new fabric.Polygon(opts.points, opts);
	    break;
	case "text":
	    // save shape
	    params.shape = "text";
	    params.text = params.text || "Double-click to add text here";
	    // for text, we generally don't want strokeWidth
	...
	JS9.Fabric._updateShape.call(this, layerName, s, null, "add");
    }
...
// call using image context
JS9.Fabric.updateShapes = function(layerName, shape, mode, nocb){
    var that = this;
    // process the specified shapes
    this.selectShapes(layerName, shape, function(obj, ginfo){
	JS9.Fabric._updateShape.call(that, layerName, obj, ginfo, mode, nocb);
    });
    return this;
};
...
// call using image context
JS9.Fabric._updateShape = function(layerName, obj, ginfo, mode, nocb){
    var i, pname, pinst, popts, xname, s, scalex, scaley, px, py, lcs;
    var display, zoom, bin, tstr, dpos, gpos, ipos, npos, objs, olen, radius;
    var pub ={};
	...
	pub.shape = "annulus";
	pub.radii = [];
	pub.imstr = sprintf("annulus(%.2f, %.2f, ", px, py);
	tstr = sprintf("annulus %s %s ", pub.x, pub.y);
	objs = obj.getObjects();
	olen = objs.length;
	for(i=0; i<olen; i++){
	    radius = objs[i].radius * scalex;
	    pub.imstr += radius.toFixed(2);
	...
	pub.imstr = sprintf("box(%.2f, %.2f, %.2f, %.2f, %.2f)", 
			      px, py, pub.width, pub.height, pub.angle);
	tstr = sprintf("box %s %s %s %s %s %s %s %s %s %s %f", 
		       pub.x, pub.y, 
		       pub.x, pub.y, 
		       pub.x + pub.width, pub.y, 
		       pub.x, pub.y, 
		       pub.x, pub.y + pub.height, 
		       pub.angle * Math.PI / 180.0);
	break;
    case "circle":
	pub.radius = obj.radius * scalex;
	pub.imstr = sprintf("circle(%.2f, %.2f, %.2f)", 
			      px, py, pub.radius);
	tstr = sprintf("circle %s %s %s %s %s %s", 
		       pub.x, pub.y, pub.x, pub.y, 
		       pub.x + pub.radius, pub.y);
	break;
    case "ellipse":
	pub.r1 = obj.width * scalex / 2;
	pub.r2 = obj.height * scaley / 2;
	pub.imstr = sprintf("ellipse(%.2f, %.2f, %.2f, %.2f, %.2f)", 
			    px, py, pub.r1, pub.r2, pub.angle);
	tstr = sprintf("ellipse %s %s %s %s %s %s %s %s %s %s %f", 
		       pub.x, pub.y, 
		       pub.x, pub.y, 
		       pub.x + pub.r1, pub.y, 
		       pub.x, pub.y, 
	...
	pub.width =  obj.width * scalex;
	pub.height = obj.height * scaley;
	pub.imstr = sprintf("point(%.2f, %.2f)", px, py);
	tstr = sprintf("point %s %s", pub.x, pub.y);
	break;
    case "polygon":
	pub.imstr = "polygon(";
	tstr = "polygon ";
	...
	for(i=0; i<obj.points.length; i++){
		...
		pub.imstr += sprintf("%.2f, %.2f", lcs.x, lcs.y);
		tstr += sprintf("%s %s", npos.x, npos.y);
		pub.pts.push(npos);
	}
	pub.imstr += ")";
        break;
    case "text":
	pub.imstr = sprintf("# text(%.2f, %.2f) text={%s}", 
			    pub.x, pub.y, obj.text);
	pub.text = obj.text;
	break;
    default:
	break;
    }
    // generate wcs string, if necessary
    if( this.wcs && (this.wcs > 0) ){
...
JS9.Fabric.addPolygonAnchors = function(dlayer, obj){
	...
	// update the shape info
	JS9.Fabric._updateShape.call(dlayer.display.image, 
				     poly.params.layerName, poly, 
				     null, "update");
	// redraw
	canvas.renderAll();
    };
...
JS9.Regions.listRegions = function(which, disp){
	...
	if( region.wcsstr && 
	    ...
	    if( lasttype !== region.imstrsys ){
		regstr += region.imstrsys;
		lasttype = region.imstrsys;
	    }
...
JS9.Magnifier.init = function(width, height){
    ...
    // set width and height on div
    this.divjq.css("width", this.width).css("height", this.height);
    // create DOM canvas element
    this.canvas = document.createElement("canvas");
    // jquery version for event handling and DOM manipulation
    this.canvasjq = $(this.canvas);
...
JS9.Panner.init = function(width, height){
    var dlayer;
    var that = this;
    // set width and height of panner
...
JS9.mouseMoveCB = function(evt){
	...
	if( (ipos.x > 0) && (ipos.y > 0) &&
		...
		if( JS9.globalOpts.internalValPos ){ im.displayValues(ipos); }
		// plugin callbacks
		for( pname in display.pluginInstances ){
		    if( display.pluginInstances.hasOwnProperty(pname) ){
			pinst = display.pluginInstances[pname];
...
JS9.instantiatePlugin = function(el, plugin, winhandle, args){
    var i, tplugin, instance, divid, divjq, pdivjq, html;
	...
	// the dhtmlwindow is the the outer div
	// NB: defined in terms of dhtmlwindow!
	instance.outerdivjq = instance.divjq.closest(".dhtmlwindow");
    } else {
	// save id
	instance.id = divjq.attr("id") || plugin.name;
	// save type
	...
	if( plugin.opts.toolbarSeparate || divjq.data("toolbarseparate") ){
	    $("<div class='drag-handle'>").insertBefore(instance.divjq);
	}
    }
	...
	if( html ){
	    ...
	    // NB: defined in terms of dhtmlwindow!
	    pdivjq = instance.divjq.closest(".drag-contentarea");
	    if( pdivjq.length === 0 ){
...
JS9.Init = function(){
	...
	// dhtmlwindow.js. We removed it from the .js file because it interered
	// with the jquery search for js9.css above. But it has to be somewhere!
	$("<div>")
    ...
    $.ajax({
      url: JS9.InstallDir(JS9.PREFSFILE),
      dataType: "json",
	async: false,
	...
	if( (JS9.BROWSER[0] === "Chrome") && (document.domain === "") ){
	    ...
	    JS9.log("could not read JS9 preference file. You are unlikely to get good results.");
	}
      }
    }); 
    // set debug flag
    ...
    JS9.RegisterPlugin("JS9", "Info", JS9.Info.init,
			...
			winTitle: "JS9 Info",
			winDims: [JS9.WIDTH, JS9.INFOHEIGHT]});
...
[[0,0,0], [0.18431, 0.18431, 0.18431], [0.37255, 0.37255, 0.37255], [0.56078, 0.56078, 0.56078], [0.74902, 0.74902, 0.74902], [0.93725, 0.93725, 0.93725], [0, 0.18431, 0.93725], [0, 0.37255, 0.74902], [0, 0.49804, 0.49804], [0, 0.74902, 0.30980], [0, 0.93725, 0], [0.30980, 0.62353, 0], [0.49804, 0.49804, 0], [0.62353, 0.30980, 0], [0.93725, 0, 0], [0.74902, 0, 0.30980]]));
	...
	set: function(args){
		...
		JS9.Load(args[i], this.display.id);
	    }
	}
    }));
    JS9.checkNew(new JS9.Command({
...
JS9.mkPublic = function(name, s){
	...
	if( JS9.Image.prototype[s] ){
	    JS9[name] = function(id){
		var myargs, got;
		var im = JS9.GetImage(id);
		if( im ){
		    // args start after command
		    myargs = Array.prototype.slice.call(arguments, 1);
		    // call the image method
		    got = im[s].apply(im, myargs);
...
JS9.mkPublic("Load", function(file, opts, func){
    var im, ext, display, localOpts;
    // sanity check
    if( !file ){
	JS9.error("JS9.Load: no file specified for image load");
	return;
    }
    // opts can be an object containing parameters, or the display string
    if( opts ){
	if( typeof opts === "function" ){
	    ...
	    if( !func && localOpts.onload ){
		func = localOpts.onload;
	    }
	} else {
	    display = opts;
	}
    ...
    // if display was passed, add it to opts
    if( !localOpts.display ){
	localOpts.display = display;
    }
    // last chance for an onload callback
    if( !func && JS9.imageOpts.onload ){
	...
	JS9.checkNew(new JS9.Image(file, localOpts, func));
	return;
    }
	...
	if( JS9.globalOpts.pngisfits ){
		...
		Fitsy.fetchURL(null, file, localOpts, JS9.NewImage);
	    } else {
	...
	if( localOpts.fits2png || 
	    ((localOpts.fits2png === undefined) && JS9.globalOpts.fits2png) ){
		...
		function(r){
			...
			if( next === "png" ){
			    ...
			    JS9.checkNew(new JS9.Image(nfile, localOpts, func));
			} else {
		...
		JS9.waiting(true);
		// ask fitsy to load the file
		Fitsy.fetchURL(file, file, localOpts, JS9.NewImage);
	    } else {
...
JS9.mkPublic("LoadWindow", function(file, opts, type, html, winopts){
    ...
    case "light":
        ...
        // and a second one for controlling the dhtml window
        did = "d" + id;
        ...
        winopts = winopts || JS9.lightOpts.dhtml.imageWin;
        // create the light window
...
JS9.mkPublic("Preload", function(arg1){
	...
	for(i=0; i<alen; i++){
		...
		JS9.preloads.push([arguments[i], arguments[j]]);
		i++;
	    } else {
		JS9.preloads.push([arguments[i], null]);
	    }
	}
	break;
    case 1:
	...
	for(i=0; i<alen; i++){
	    j = i + 1;
	    if( (j < alen) && (typeof arguments[j] === "object") ){
		try{ JS9.Load(arguments[i], arguments[j]); }
		catch(e){ emsg = emsg + " " + arguments[i]; }
		i++;
	    } else {
		try{ JS9.Load(arguments[i], null); }
		catch(e){ emsg = emsg + " " + arguments[i]; }
	    }
	}
	JS9.globalOpts.alerts = true;
	...
	for(i=0; i<JS9.preloads.length; i++){
	    try{ JS9.Load(JS9.preloads[i][0], JS9.preloads[i][1]); }
...
JS9.mkPublic("GetImageData", function(id, dflag){
    var im = JS9.GetImage(id);
...
JS9.mkPublic("LoadAuxFile", function(id, s, func){
    var im = JS9.GetImage(id);
    if( im ){
	im.loadAuxFile(s, func);
    } else {
	JS9.error("could not find image to refresh");
    }
});
    
// run analysis from a Web page form
JS9.mkPublic("SubmitAnalysis", function(el, aname, id, func){
    var topjq, formjq, dispid, im, obj, errstr;
    var a = JS9.lightOpts[JS9.LIGHTWIN];
    // if analysis name was not passed, it was saved in the light window div
    if( aname ){
	dispid = id || JS9.lookupDisplay().id;
    } else {
...
JS9.mkPublic("PixToWCS", function(id, ix, iy){
    var s, arr;
    var x = 1.0;
    var im = JS9.GetImage(id);
    if( im ){
...
JS9.mkPublic("WCSToPix", function(id, ra, dec){
    var s, arr;
    var im = JS9.GetImage(id);
    if( im ){
	if( im.wcs > 0 ){
...
JS9.mkPublic("NewShapeLayer", function(id, layer, opts){
    var im = JS9.GetImage(id);
    if( im ){
...
// add a region
JS9.mkPublic("AddRegions", function(id, region, opts){
    var im = JS9.GetImage(id);
    if( im ){
...
JS9.mkPublic("RemoveRegions", function(id, region){
    var im = JS9.GetImage(id);
    if( im ){
...
JS9.mkPublic("GetRegions", function(id , region){
    var im = JS9.GetImage(id);
    if( im ){
...
JS9.mkPublic("ChangeRegions", function(id, region, opts){
    var im  = JS9.GetImage(id);
    if( im ){
